...
- 后端现在默认由 Agent 正常直接开发；除非用户明确要求“后端不要直接修改/我自己写”，否则按任务书直接修改后端业务代码、测试代码和必要配置，并运行相关验证命令。
...
| 阶段 14：部署与运维 | 当前阶段 | 生产配置、日志监控、备份恢复、启动复现、构建产物和部署脚本 |
...
阶段 14 当前开始。优先做可复现的本地/生产配置、启动脚本、Docker Compose、构建产物说明、数据库备份恢复和 README 收尾；如果只是课程或答辩项目，先保证一键复现和演示稳定，不急着上云。
