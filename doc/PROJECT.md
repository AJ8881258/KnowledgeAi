...
项目当前处于 **阶段 15：知识库质量与文档处理增强规划中**。阶段 14 已完成 Docker 化与运维收尾，当前版本已经具备从本地开发到 Docker 全量启动的完整复现路径，并具备健康检查、日志排查、环境变量管理和 PostgreSQL 备份恢复说明；`do.md` 作为本地任务输入文件已加入 `.gitignore`，不再作为项目文件提交。
...
- 阶段 0-14 已完成。
- 阶段 15 规划中，目标是增强文档处理质量可视化、失败重试、重新处理、文档摘要和检索命中解释。
