...
阶段 10：RAG 体验增强已完成。当前文件保留阶段 10 验收结果和下一阶段占位；进入阶段 11 前，由文档会话重新改写本文件为“阶段 11 前端任务书”。

阶段 10 已完成的前端目标：增强 Chat 问答体验，包括发送中状态、空检索降级、模型失败提示、引用来源展示、多轮上下文说明。流式输出未实现，继续作为后续规划；除非 `doc/API.md` 已补充并确认 SSE 契约，否则不要主动实现流式 UI。
...
- `pnpm build` 已通过。
- 目标文件 ESLint 已通过：`src/components/chat-page`、`src/api/chat.ts`、`src/pages/Chat.tsx`。
- 发送问题时不会重复提交，发送中状态清晰。
- 空引用来源不是错误状态，前端不会伪造 sources。
- 引用来源长文本支持截断/展开，不会撑破布局。
- 没有新增直接 `fetch`。
- 没有新增组件级 `localStorage`。
- 没有新增 mock-only Chat 逻辑。
- 未经用户明确要求，未主动使用浏览器工具验收。

## 下一阶段占位

下一阶段为阶段 11：文档处理增强。进入阶段 11 前，文档会话需要根据 `doc/STAGE_PLAN.md`、`doc/PROJECT.md` 和 `doc/API.md` 重新写清前端任务边界，避免前端会话继续重复阶段 10 任务。
