...
- Stage planning source of truth:
  ...
  - When dispatching implementation, review, debugging, or documentation tasks to Codex Threads or `AGENT_TEAM`, set the worker thinking/reasoning level to the highest available level, such as `xhigh` / 最高级, unless the user explicitly asks to reduce cost or reasoning depth. Worker prompts must define role, objective, allowed scope, forbidden scope, required verification, expected handoff format, and the rule that they must not overwrite or revert others' work.
...
- 如果用户把当前会话定义为“文档/规划/指挥/交接会话”，当前会话只能同步 `doc/` 核心文档、`AGENTS.md` 规则和前后端任务书，不得继续修改 `src/`、`backend/` 等业务代码。即使计划里包含前后端修复，也应写入 `doc/FRONTEND_TASK.md`、`doc/BACKEND_TASK.md`；后续需要实际开发时，推荐采用团队编排模式：主对话担任项目经理/协调者，负责审核计划、拆分任务、下发 Codex 后台 Thread/worktree 或 `AGENT_TEAM`、集成结果和最终验证。复杂阶段、多子系统任务或容易让单条对话积累过多 WindowsContext 的任务，优先拆分到多个 Codex 后台 Thread/worktree 或 `AGENT_TEAM` 子任务，并由协调者负责约束范围、集成、验证和汇总，不再默认要求用户把前端/后端提示复制到新会话。协调者下发 Thread 或 `AGENT_TEAM` 任务时，必须把 worker 思考/推理等级设为可用最高级（例如 `xhigh` / 最高级），除非用户明确要求降低成本或降低思考等级。
...
- 当前端验收需要真实页面行为时，协调者可以下发专门的 Codex Thread/worktree、`AGENT_TEAM` 任务，或在本线程使用 `browser-use`、Browser、Chrome、Playwright、截图等浏览器工具做 UI/视觉验收；实现线程如果没有被明确分配浏览器验收任务，仍以 `pnpm build`、目标 ESLint、类型检查等代码级验证为主。
